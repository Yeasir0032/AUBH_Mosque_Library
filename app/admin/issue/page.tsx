...
export default function IssueBookPage() {
  ...
  const handleIssueBook = async (e: React.FormEvent) => {
    ...
    try {
      // Step 1: Resolve User ID from Mobile
      const { data: userData } = await axios.get(`/api/admin/resolve-user?mobile=${mobile}`);
      if (!userData || !userData.id) throw new Error("User not found with this mobile number");
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 pt-12 animate-in slide-in-from-bottom-4 duration-500">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500 dark:text-zinc-400">Manually assign a book to a user account using their registered mobile number.</p>
      </div>
      ...
      <div className="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-3xl shadow-sm p-8 md:p-12">
        <form onSubmit={handleIssueBook} className="space-y-6">
          ...
          <div>
            <label htmlFor="mobile" className="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-2">
              User Mobile Number
            </label>
